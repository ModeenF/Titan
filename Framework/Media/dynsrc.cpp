...
//NAME("Audio Synthesizer Filter"), LPUNKNOWN lpunk, CLSID_SynthFilter, HRESULT *phr
...
HRESULT CDynamicSource::AddPin(CDynamicSourceStream *pStream)
{
    ...
    CDynamicSourceStream **paStreams = EM_new CDynamicSourceStream *[m_iPins + 1];
    if (paStreams == NULL) {
...
//	if(*(pMediaType -> Type()) == MEDIATYPE_Audio || *(pMediaType -> Type()) == MEDIATYPE_AnalogAudio)
//	{
/*		if(*(pMediaType -> Subtype()) == FORMAT_WaveFormatEx)
		{
			WAVEFORMATEX sFormat = *((WAVEFORMATEX*) pMediaType -> Subtype());

			if(sFormat.wFormatTag == WAVE_FORMAT_PCM)
			{
				MessageBox(NULL, "Format is Wave Format PCM", "", MB_OK);
			}
			else
			{
				MessageBox(NULL, "Format is other than Wave Format PCM", "", MB_OK);
			}
		} */
//	}
...
DWORD CDynamicSourceStream::ThreadProc(void) {
    ...
    do {
        com = GetRequest();
        if (com != CMD_INIT) {
			//eo << "   DirectX -> ERROR! Request not INIT!?!" << ef;
