...
CommandZoomInTrackVertical::CommandZoomInTrackVertical(TrackWindow* p_opTrackWindow)
{
	m_opTrackWindow = p_opTrackWindow;
}

void CommandZoomInTrackVertical::ExecuteE()
{
	int32 vX;
	int32 vY;
	bool vButtonOne;
	...
	if(opTrackView -> GetView() -> LockWindow())
	{
		...
		if(opTrack -> IncreaseZoomScale())
		{
			opTrackInfo -> CalculateTrackInfoFramesAndUpdate(opTrack);
...
bool CommandZoomInTrackVertical::IsUndoable()
{
	return false;
}

bool CommandZoomInTrackVertical::RequiresParameters()
{
	return false;
}
